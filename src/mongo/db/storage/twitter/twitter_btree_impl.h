...
namespace mongo {
    ...
    class TwitterRecoveryUnit;

    class TwitterBtreeBuilderImpl : public BtreeBuilderInterface {
    public:
        virtual Status addKey(const BSONObj& key, const DiskLoc& loc) = 0;
        virtual unsigned long long commit(bool mayInterrupt) = 0;
    };

    class TwitterBtreeImpl : public BtreeInterface {
    public:
        TwitterBtreeImpl();
    ...
    private:
        TwitterRecoveryUnit* _getRecoveryUnit( OperationContext* opCtx ) const;
    };
}
